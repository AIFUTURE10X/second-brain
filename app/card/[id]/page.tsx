...
// Client-side "is this a video card" check. The route re-checks authoritatively
// with extractYouTubeId; this only decides whether to show the section, and is
// kept local so the server-only youtube module stays out of the client bundle.
...
export default function CardPopoutPage() {
  ...
  const isVideoCard = YOUTUBE_URL_RE.test(form.url || "");
  ...
  // Only video cards ever ask for a transcript, and only once the item has
  // loaded — nothing else on the page needs the text.
  const loadTranscript = useCallback(async () => {
    if (!id || !item?.url || !YOUTUBE_URL_RE.test(item.url)) return;
    setTranscriptLoading(true);
    try {
      const res = await fetch(`/api/transcript?itemId=${encodeURIComponent(id)}`);
      if (res.ok) {
  ...
  const removeTranscript = async () => {
    ...
    try {
      const res = await fetch(`/api/transcript?itemId=${encodeURIComponent(id)}`, { method: "DELETE" });
  ...
  return (
    <div className="min-h-screen bg-brand-dark text-gray-200 p-5 max-w-2xl mx-auto">
      ...
      {/* Transcript — video cards only. Collapsed by default: transcripts run to
          tens of thousands of characters and would bury the rest of the form. */}
      {isVideoCard && (
        <div className="mb-4 rounded-lg border border-brand-border bg-brand-muted">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-brand-border">
            <span className="text-[11px] font-mono text-gray-400 tracking-wide">Transcript</span>
            {transcript && (
            ...
            {transcript && (
              <button
                ...
                className="text-[11px] font-mono text-gray-500 hover:text-gray-300 transition"
              >{transcriptOpen ? "Hide" : "Show"}</button>
            )}
            <button
              type="button"
            ...
            >
              {transcriptBusy && <span className="inline-block w-3 h-3 border border-current border-t-transparent rounded-full" style={{ animation: "spin 0.6s linear infinite" }} />}
              {transcriptBusy ? "Fetching…" : transcript ? "Re-fetch" : "Fetch transcript"}
            </button>
          </div>

          {transcriptLoading && (
            <div className="px-3 py-2 text-[11px] font-mono text-gray-600">Loading…</div>
          )}

          {!transcriptLoading && !transcript && (
            <div className="px-3 py-2 text-[11px] font-mono text-gray-600">
          ...
          {transcript && transcriptOpen && (
            <>
              <textarea
                ...
                rows={12}
                className="w-full px-3 py-2 bg-transparent text-[12px] text-gray-400 outline-none resize-y leading-relaxed font-mono"
              />
              <div className="flex items-center gap-3 px-3 py-2 border-t border-brand-border">
                <button
                  type="button"
