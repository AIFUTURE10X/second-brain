...
// Video transcripts live in their own table rather than on `items`: they run to
// tens of thousands of characters, and the grid's list read selects every item
// column, so storing them inline would drag that payload down for every card.
// Fetched on demand, one row per card.
export const itemTranscripts = pgTable("item_transcripts", {
  itemId: uuid("item_id").primaryKey().references(() => items.id, { onDelete: "cascade" }),
  text: text("text").notNull().default(""),
  // "youtube-captions" (free) or "apify" (paid fallback) — surfaced in the UI so
  // it's clear where a transcript came from.
  source: text("source").notNull().default(""),
  language: text("language"),
  isAutoGenerated: boolean("is_auto_generated").notNull().default(false),
  charCount: integer("char_count").notNull().default(0),
  fetchedAt: timestamp("fetched_at", { mode: "date" }).defaultNow().notNull(),
  // Weight D — a transcript is bulk source material and must never outrank a
  // title match. Named transcript_tsv rather than search_tsv so the two stay
  // unambiguous when search joins this table to items. left() caps the input:
  // a tsvector tops out at 1MB, and a row that exceeded it couldn't be stored.
  transcriptTsv: tsvector("transcript_tsv").generatedAlwaysAs(
    sql`setweight(to_tsvector('english', left(coalesce(text, ''), 400000)), 'D')`
  ),
}, (table) => [
  index("item_transcripts_tsv_idx").using("gin", table.transcriptTsv),
]);

export type ItemTranscript = typeof itemTranscripts.$inferSelect;
